...
def _write_heal_metrics(db_path: Path, date: str, report: HealReport, oc_home: Path) -> None:
    """Write heal diagnostic results into goal_metrics for Dashboard display."""
    import sqlite3
    ...
    # Token cost from cost_analysis action
    ...
    if mem_action:
        import re
        m = re.search(r"(\d+)\s*组", mem_action.detail)
    ...
    # Overall heal score: safe_executed / total_actions
